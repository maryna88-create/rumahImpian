import { a as gotoBlock, c as uniqArray, i as bodyUnlock, n as bodyLockStatus, o as slideToggle, r as bodyLockToggle$1, s as slideUp, t as bodyLock } from "./common.min.js";
...
function initSliders() {
	...
	if (slider) initSwiperClickNavigation({
		swiper: new Swiper(slider, {
			...
			navigation: {
				prevEl: ".controls-features__arrow--prev",
				nextEl: ".controls-features__arrow--next"
			},
			on: {
				init(swiper) {
					requestAnimationFrame(() => {
						const realSlides = getRealSlides(swiper);
						const activeSlide = realSlides.find((slide) => slide.dataset.active === "true");
						if (activeSlide) {
							const index = realSlides.indexOf(activeSlide);
							swiper.slideToLoop(index, 800, false);
						} else swiper.slideToLoop(0, 0, false);
		...
		containerSelector: ".features__wrapper",
		itemSelector: ".features__slide"
	});
}
function getRealSlides(swiper) {
	return [...swiper.slides].filter((s) => !s.classList.contains("swiper-slide-duplicate"));
}
function updateActiveSlide(swiper) {
	...
	if (activeSlide) activeSlide.classList.add("features__slide--active");
}
function initSwiperClickNavigation({ swiper, containerSelector, itemSelector }) {
	...
	container.addEventListener("click", (e) => {
		...
		swiper.slideToLoop(realIndex, 800, false);
	});
}
...
//#endregion
//#region src/components/custom/overlay/overlay.js
var overlayController = (() => {
	...
	function open(type) {
		...
		overlay.classList.remove("overlay--menu", "overlay--popup", "overlay--search");
		overlay.classList.add(`overlay--${type}`);
	}
	function close() {
		if (!overlay) return;
		const wasMenuOpen = document.documentElement.hasAttribute("data-fls-menu-open");
		overlay.classList.remove("overlay--active", "overlay--menu", "overlay--popup", "overlay--search");
		if (wasMenuOpen) {
...
function menuInit() {
	...
	document.addEventListener("click", function(e) {
		...
		if (bodyLockStatus && menuBtn) {
			...
			if (document.documentElement.hasAttribute("data-fls-menu-open")) overlayController.open("menu");
			else overlayController.close();
		}
	});
	...
	document.addEventListener("click", (e) => {
		if (e.target.closest("[data-fls-scrollto]")) closeMenu();
	});
	function closeMenu() {
...
["load", "resize"].forEach((evt) => window.addEventListener(evt, setHeaderHeightVar));
//#endregion
...
function initActiveCard({ itemSelector = ".works__item", innerSelector = null, activeClass = "--active", onActivate = null, onDeactivate = null, dragThreshold = 5 } = {}) {
	const items = document.querySelectorAll(itemSelector);
	if (!items.length) return;
	const getEl = (item) => innerSelector ? item.querySelector(innerSelector) : item;
	let defaultIndex = [...items].findIndex((item) => item.dataset.popular === "true");
	...
	items.forEach((item, index) => {
		...
		item.addEventListener("pointermove", (e) => {
			if (Math.abs(e.clientX - startX) > dragThreshold) isDragging = true;
		});
...
function initTabs({ tabSelector, activeClass, inputSelector = null, dataAttr = "type", onChange = null, defaultByDataAttr = null } = {}) {
	const tabs = document.querySelectorAll(tabSelector);
	const input = inputSelector ? document.querySelector(inputSelector) : null;
	if (!tabs.length) return;
	let defaultTab = null;
	if (defaultByDataAttr) defaultTab = [...tabs].find((tab) => tab.dataset[dataAttr] === defaultByDataAttr);
	if (!defaultTab) defaultTab = tabs[0];
	tabs.forEach((t) => t.classList.remove(activeClass));
	defaultTab.classList.add(activeClass);
	if (input) input.value = defaultTab.dataset[dataAttr];
	if (onChange) onChange(defaultTab, defaultTab.dataset[dataAttr]);
	tabs.forEach((tab) => {
		tab.addEventListener("click", () => {
			tabs.forEach((t) => t.classList.remove(activeClass));
			tab.classList.add(activeClass);
			if (input) input.value = tab.dataset[dataAttr];
			if (onChange) onChange(tab, tab.dataset[dataAttr]);
		});
	});
}
function initCardsReveal() {
	const list = document.querySelector(".recomendations__cards");
	const items = document.querySelectorAll(".recomendations__card");
	const button = document.querySelector(".recomendations__button");
	if (!list || !items.length || !button) return;
	let expanded = false;
	let visibleCount = 0;
	function applyLayout() {
		const isMobile = window.matchMedia("(max-width: 509px)").matches;
		const isTablet = window.matchMedia("(max-width: 797px)").matches;
		items.forEach((item) => {
			item.style.display = "";
			item.classList.remove("show", "mobile-visible");
		});
		if (isMobile) {
			items.forEach((item) => item.classList.add("mobile-visible"));
			button.style.display = "none";
			return;
		}
		visibleCount = isTablet ? 3 : 6;
		items.forEach((item, index) => {
			if (index < visibleCount) item.classList.add("show");
		});
		button.style.display = "block";
		expanded = false;
		button.textContent = "Lihat Lainnya...";
	}
	function showMore() {
		const isTablet = window.matchMedia("(max-width: 797px)").matches;
		const columns = getGridColumns(list);
		visibleCount += isTablet ? 3 : Math.max(columns, 3);
		items.forEach((item, index) => {
			if (index < visibleCount) item.classList.add("show");
		});
	...
	function hideBack() {
		const baseCount = window.matchMedia("(max-width: 797px)").matches ? 3 : 6;
		const currentTop = list.getBoundingClientRect().top + window.scrollY;
		items.forEach((item, index) => {
			if (index < baseCount) item.classList.add("show");
	...
	function getGridColumns(gridElement) {
		...
		if (width <= 797 && width >= 510) return 1;
...
initActiveCard({
	itemSelector: "[data-fls-itemservices]",
	activeClass: "item-services--active"
});
document.addEventListener("DOMContentLoaded", () => {
	initTabs({
		tabSelector: ".search-tabs__item",
		activeClass: "search-tabs__item--active",
		inputSelector: "#searchMode",
		dataAttr: "type",
		defaultByDataAttr: "jual"
	});
	initCardsReveal();
});
initActiveCard({
	itemSelector: "[data-fls-card]",
